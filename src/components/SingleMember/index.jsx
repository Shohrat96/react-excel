...
const FlightsTable = ({ data, headers, member }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      const hasNAJ = Object.values(row).some(
        (value) => String(value).trim().toUpperCase() === "NAJ"
      );
      ...
      <div className={styles.controlsContainer}>
        ...
        <div className={styles.onlyAlerts}>
          <CustomSliderComponent
            active={showWarningsOnly}
            toggleActive={toggleWarningsFilter}
            activeTitle="Only Alerts"
            deactiveTitle="All Flights"
          />
        </div>
      </div>
          ...
          {notifications.map((notif) => (
            <div
              ...
              className={`${styles.notificationBox} ${
                notif.type === "warning" ? styles.warningNotification : styles.infoNotification
              }`}
            >
              {notif.message}
            </div>
              ...
              <tr key={index} className={row.isWarning ? styles.warningRow : ""}>
                {Object.keys(headers).map((value) => {
                  if (
                    ...
                    return <td key={value}>{formatTAFData(row[value])}</td>;
                  }
